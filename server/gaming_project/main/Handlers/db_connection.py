...
def get_db():
    ...
    if _client is None:
        ...
        try:
            ...
            # breaks on Windows: pymongo 3.11's bundled OpenSSL can negotiate a handshake
            # Atlas's TLS termination rejects outright (TLSV1_ALERT_INTERNAL_ERROR on every
            # shard — confirmed live against QuickPrint's identical setup) well before
            # certificates are even compared. This fixes the handshake itself, not just
            # certificate trust, and does NOT weaken verification in any way (still full
            # hostname + chain validation, just against a known-good, regularly-updated
            # bundle instead of whatever the OS happens to have). Render's Linux containers
            # never hit this, so production is unaffected either way — this is purely for
            # local Windows dev reliability.
