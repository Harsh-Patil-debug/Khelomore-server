...
# Regression tests for the second-factor "payment password" that gates a cafe owner's
# access to entering/viewing their payment gateway credentials in cafe-command-center:
# required at admin signup (separate from the login password), a one-time set-password
# path for accounts that predate this feature, a verify-password unlock with the same
# lockout behavior as login, a super-admin bypass, and the PUT/DELETE credential
# endpoints actually re-checking it server-side (not just a UI gate).

from ..Handlers import auth_handler
from .base import SecurityTestCase


class AdminSignupPaymentPasswordTests(SecurityTestCase):
    def _register(self, email, password, payment_credentials_password, role="admin", gamertag="TestOwner", phone=""):
        parts = [gamertag, email, password]
        if phone:
            parts.append(phone)
        if payment_credentials_password is not None:
            parts.append(payment_credentials_password)
        *enc_parts, iv = self.encrypt_with_shared_iv(*parts)
        ...
        if phone:
            ...
            idx += 1
        if payment_credentials_password is not None:
            body["payment_credentials_password"] = enc_parts[idx]
        return self.client.post("/api/v1/main/auth/register/", body, format="json")

    def test_admin_signup_without_payment_password_is_rejected(self):
        owner_email = self.unique_email("owner")
        self.make_cafe(owner_email=owner_email)
        resp = self._register(owner_email, "CorrectHorseBattery1", payment_credentials_password=None)
        self.assertEqual(resp.status_code, 400)
        self.assertIn("payment password", resp.json().get("error", "").lower())

    def test_admin_signup_rejects_payment_password_same_as_login_password(self):
        owner_email = self.unique_email("owner")
        self.make_cafe(owner_email=owner_email)
        resp = self._register(owner_email, "CorrectHorseBattery1", payment_credentials_password="CorrectHorseBattery1")
        self.assertEqual(resp.status_code, 400)
        self.assertIn("different", resp.json().get("error", "").lower())

    def test_admin_signup_succeeds_and_hashes_payment_password_separately(self):
        owner_email = self.unique_email("owner")
        self.make_cafe(owner_email=owner_email)
        resp = self._register(owner_email, "CorrectHorseBattery1", payment_credentials_password="TotallyDifferentPass2")
        self.assertEqual(resp.status_code, 200, resp.content)

        admin_doc = self.db.admins.find_one({"email": owner_email})
        self.track("admins", admin_doc["_id"])
        self.assertIn("payment_credentials_password_hash", admin_doc)
        self.assertNotEqual(admin_doc["payment_credentials_password_hash"], admin_doc["password_hash"])
        self.assertTrue(auth_handler.verify_password(admin_doc["payment_credentials_password_hash"], "TotallyDifferentPass2"))

    def test_gamer_signup_does_not_require_payment_password(self):
        resp = self._register(self.unique_email("gamer"), "CorrectHorseBattery1", payment_credentials_password=None, role="")
        self.assertEqual(resp.status_code, 200, resp.content)


class PaymentPasswordGateTests(SecurityTestCase):
    def setUp(self):
        super().setUp()
    ...
    def _set_password_hash(self, password="MySecretPayPass1"):
        self.db.admins.update_one({"email": self.owner_email}, {"$set": {"payment_credentials_password_hash": auth_handler.ph.hash(password)}})
        return password

    def test_status_reports_no_password_for_a_pre_existing_account(self):
        resp = self.client.get(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/password-status/", **self.auth_header(self.owner_token)
        )
    ...
    def test_verify_fails_with_no_password_set_yet(self):
        resp = self.client.post(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/verify-password/",
            {"password": "whatever"},
            format="json",
            **self.auth_header(self.owner_token),
        )
    ...
    def test_saving_credentials_without_password_is_rejected(self):
        ...
        resp = self.client.put(
            ...
            {"client_id": "cf_test_abc123", "client_secret": "supersecret"},
            format="json",
            **self.auth_header(self.owner_token),
        )
        self.assertEqual(resp.status_code, 403)
    ...
    def test_saving_credentials_with_correct_password_succeeds(self):
        ...
        resp = self.client.put(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/",
            {"client_id": "cf_test_abc123", "client_secret": "supersecret", "payment_password": pw},
            format="json",
            **self.auth_header(self.owner_token),
        )
        self.assertEqual(resp.status_code, 200)
...
class PaymentPasswordForgotResetTests(SecurityTestCase):
    """A forgotten payment password used to be a permanent lockout (no recovery path at
    ...
    def _forgot(self, token):
        return self.client.post(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/forgot-password/",
            {}, format="json", **self.auth_header(token),
        )

    def _reset(self, otp, new_password, token=None):
        return self.client.post(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/reset-password/",
    ...
    def test_owner_can_request_a_reset_code(self):
        ...
        self.assertIsNotNone(admin_doc.get("payment_credentials_reset_otp_code"))
    ...
    def test_a_different_authenticated_owner_cannot_request_a_reset_for_this_cafe(self):
        ...
        # forgot_payment_credentials_password_handler — a non-owner can't call this
        # endpoint for a cafe_id they don't own at all, regardless of what handler-level
        # checks exist.
        resp = self._forgot(other_token)
        self.assertEqual(resp.status_code, 403)
        admin_doc = self.db.admins.find_one({"email": self.owner_email})
        self.assertIsNone(admin_doc.get("payment_credentials_reset_otp_code"))
    ...
    def test_correct_otp_resets_and_new_password_unlocks(self):
        self._set_password_hash("OldPayPass1")
        self._forgot(self.owner_token)
        otp = self.db.admins.find_one({"email": self.owner_email})["payment_credentials_reset_otp_code"]

        resp = self._reset(otp, "BrandNewPayPass2")
        self.assertEqual(resp.status_code, 200)

        unlock_resp = self.client.post(
            f"/api/v1/main/cafes/{self.cafe_id}/payment-credentials/verify-password/",
            {"password": "BrandNewPayPass2"},
            format="json",
            **self.auth_header(self.owner_token),
        )
    ...
    def test_reset_works_even_if_no_password_was_ever_set(self):
        ...
        self._forgot(self.owner_token)
        otp = self.db.admins.find_one({"email": self.owner_email})["payment_credentials_reset_otp_code"]
        resp = self._reset(otp, "FirstEverPayPass1")
        self.assertEqual(resp.status_code, 200)
    ...
    def test_reset_clears_an_existing_verify_lockout(self):
        ...
        admin_doc = self.db.admins.find_one({"email": self.owner_email})
        self.assertIsNotNone(admin_doc.get("payment_credentials_password_locked_until"))

        self._forgot(self.owner_token)
        otp = self.db.admins.find_one({"email": self.owner_email})["payment_credentials_reset_otp_code"]
        self._reset(otp, "BrandNewPayPass2")

        admin_doc = self.db.admins.find_one({"email": self.owner_email})
        self.assertIsNone(admin_doc.get("payment_credentials_password_locked_until"))
        self.assertIsNone(admin_doc.get("payment_credentials_password_attempts"))

    def test_wrong_otp_is_rejected_and_counts_as_an_attempt(self):
        self._forgot(self.owner_token)
        resp = self._reset("000000", "BrandNewPayPass2")
        self.assertEqual(resp.status_code, 400)
        admin_doc = self.db.admins.find_one({"email": self.owner_email})
        self.assertEqual(admin_doc.get("payment_credentials_reset_otp_attempts"), 1)

    def test_too_many_wrong_attempts_locks_out_the_reset_code(self):
        self._forgot(self.owner_token)
        for _ in range(auth_handler.MAX_OTP_ATTEMPTS):
            self._reset("000000", "BrandNewPayPass2")
        admin_doc = self.db.admins.find_one({"email": self.owner_email})
        self.assertIsNone(admin_doc.get("payment_credentials_reset_otp_code"))
    ...
    def test_new_payment_password_same_as_login_password_is_still_allowed_here(self):
